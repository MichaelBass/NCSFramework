...
-(ItemData*) getItem :(NSString*) ID
{
    ...
    if ([self.ItemResponseList count] > 29) {
        return nil;
    }
...
-(NSString *)getPreviousItem
{
    ...
    Result* myResult = [self.ItemResponseList objectAtIndex: pos -1];
    
    
    nextItem = myResult.ItemID;
    self.Ability = myResult.Score;
    self.SE = myResult.SE;
    ...
    for(int i = 0; i < keys.count; i++){
        ...
        if([myItemCalibration.ItemID isEqualToString:nextItem] ){
            myItemCalibration.Administered = FALSE;
...
- (double) estimateTheta:(int)response responsetime:(double)responsetime
{
    ...
    if(_correctAnswered == 0 ){
        ...
        */
        ...
        ((Result*)[self.ItemResponseList objectAtIndex: pos -1]).Score = self.Ability;
        ((Result*)[self.ItemResponseList objectAtIndex: pos -1]).SE = self.SE;
        //NSLog(@"itemID: %@  self.ability: %f -- self.SE: %f",self.itemID, self.Ability, self.SE);
    ...
    do {
        ...
        if (fabsf(change) <= Convergence) {
            converged = YES;
        }
        if (fabsf(change) > MaxStepChange) {
...
- (ItemData*) processResponse:(int)response responsetime:(double)responsetime
{
   ...
   // NSLog(@"response: %d... response time: %f...", response, responsetime);
    ...
    for(Element* e in myItem.elements)
    {
        for(Map* m in e.mappings)
        {
            if( [ m.Value isEqualToString:[[NSNumber numberWithInt:response] stringValue]] ){
                ...
                if ([m.Description isEqual:@"1"]){
                    answer = 1;
                }
                
                myItem.ResponseDescription = m.Value;
                break;
            }
        }
    }
